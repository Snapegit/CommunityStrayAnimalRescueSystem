	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.jiuzhuzhanmingcheng" placeholder="救助站名称"
								clearable>
							</el-input>
						</div>
					</div>
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.dizhi" placeholder="地址"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('jiuzhuzhan','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('jiuzhuzhan','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('jiuzhuzhan','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('jiuzhuzhan','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('jiuzhuzhan','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.jiuzhuzhanmingcheng&&searchQuery.value.jiuzhuzhanmingcheng!=''){
			params['jiuzhuzhanmingcheng'] = '%' + searchQuery.value.jiuzhuzhanmingcheng + '%'
		}
		if(searchQuery.value.dizhi&&searchQuery.value.dizhi!=''){
			params['dizhi'] = '%' + searchQuery.value.dizhi + '%'
		}
		context?.$http({
			url: `${tableName}/page`,
			method: 'get',
