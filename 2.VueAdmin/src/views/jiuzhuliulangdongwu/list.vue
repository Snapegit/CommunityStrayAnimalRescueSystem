	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
					...
					<div class="search_view">
						...
						<div class="search_box">
							<el-select
								class="search_sel"
								clearable
								v-model="searchQuery.dongwupinzhong" 
								placeholder="动物品种"
								>
								<el-option v-for="item in dongwupinzhongLists" :label="item" :value="item"></el-option>
							</el-select>
						</div>
					</div>
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.didian" placeholder="地点"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('jiuzhuliulangdongwu','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('jiuzhuliulangdongwu','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('jiuzhuliulangdongwu','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('jiuzhuliulangdongwu','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				ref="table"
				v-if="btnAuth('jiuzhuliulangdongwu','查看')"
				:data="list"
				...
				<el-table-column label="审核" v-if="btnAuth('jiuzhuliulangdongwu','审核')" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('jiuzhuliulangdongwu','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('jiuzhuliulangdongwu','查看评论')" type="warning" @click="commentClick(scope.row.id)">查看评论</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	//基础信息
	const tableName = 'jiuzhuliulangdongwu'
	const formName = '救助流浪动物'
	...
	const getList = () => {
		...
		if(searchQuery.value.dongwupinzhong&&searchQuery.value.dongwupinzhong!=''){
			params['dongwupinzhong'] = searchQuery.value.dongwupinzhong
		}
		if(searchQuery.value.didian&&searchQuery.value.didian!=''){
			params['didian'] = '%' + searchQuery.value.didian + '%'
		}
	...
	const approvalClick = (row) => {
		let params = {
			id:row.id,
			biaoti: row.biaoti,
			dongwupinzhong: row.dongwupinzhong,
			didian: row.didian,
			tupian: row.tupian,
	...
	const commentClick=(id)=>{
		context?.$router.push('/discussjiuzhuliulangdongwu?refid=' + id)
	}
	//查询审核状态列表
